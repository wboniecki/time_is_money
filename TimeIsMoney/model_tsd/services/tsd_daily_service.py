...
class TSDDailyService:
    def create(self, _itemId, _realmName):
        new_tsd = ItemRealmTimeSeriesDataDaily()
    ...
    def _getTSD(self, _date, _itemId, _realmName):
        item_service = ItemService()
        realm_service = RealmService()
        return ItemRealmTimeSeriesDataDaily.objects.filter(date=_date, item=item_service.getItemDBId(_itemId), connected_realm=realm_service.getRealmConnectedRealmId(_realmName)).first()
